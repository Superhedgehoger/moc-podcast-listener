...
The script writes a transcript, timestamp segments, metadata, archived Show Notes, a media manifest, and an `_Agent任务指令.txt` file. Follow the generated instruction file to finish the report.
...
- Keep the transcript as a separate source document. In the report, describe it and link the transcript, segments JSON, and SRT with paths relative to the report; never embed the complete transcript.
...
1. Confirm the report file exists.
...
5. Confirm the report's transcript, segments, and SRT links resolve.
...
- Footer: original episode URL plus relative paths to segments JSON, SRT, metadata, Show Notes, and the media manifest, followed by an explicit end marker.
