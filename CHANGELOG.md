...
- Apple Podcasts 与 iTunes 目录结果回查发布方 RSS，保留 Podcasting 2.0 Transcript、章节和语言元数据
- 原始官方字幕文件保存到每期资料包的 `转录数据/`，标准化转录稿和 metadata 记录来源 URL、文件路径及人工/自动类型

## v4.15.1 (2026-08-29)
- 新总结稿标题后新增 `转录总结日期`，严格核验要求使用完成转录总结的本地日期而非节目发布日期
- 新增历史总结日期回填工具：优先采用作业核验时间，缺失时回退报告修改时间，并在写入前自动备份
- Show Notes 在原有 `shownotes.md` 内新增人类可读「链接归档」，同时继续写入 manifest，不增加额外阅读文件
- 修复纯文本 URL 吞入中文右括号、分号和后续句子的问题；新总结漏掉 Show Notes 外链时核验失败
- 新增历史 Show Notes 链接回填工具，清洗 URL、补齐链接列表并保留在线地址；网页快照仍按需启用

## v4.15.0 (2026-08-29)
- 新增 Obsidian、Notion、Zotero CSL-JSON（洞察写入 abstract）、NotebookLM 和 MCP/Agent 友好导出
- 导出内容全部由本地知识索引重建，不改变或替代原始转录稿、总结稿、资料包和个人笔记
- 新增统一 `--export` 命令、独立 `podcast-export.py` 和导出 manifest

## v4.14.0 (2026-08-29)
- 新增 RSS 订阅配置、去重状态、关键词/优先级评分和每日 Brief
- 订阅扫描只读取 feed 元数据和发布方 transcript 标记，不自动下载音频或批量运行 ASR
- 订阅数据归入 `资料/订阅/`，筛选结果归入 `资料/Brief/`，保持根目录清爽

## v4.13.0 (2026-08-29)
- 新增 `资料/knowledge-index.jsonl`，聚合单集主题、人物、AI 标签、用户标签、洞察和本地路径
- 新增 `podcast-search.py`，支持关键词、人物、标签和日期过滤，无需向量数据库
- 新增 `--rebuild-knowledge-index`，可为历史资料补齐知识草稿和个人笔记模板

## v4.12.0 (2026-08-29)
- 每期资料包新增 `我的笔记.md`，供用户记录评论、认同/反对、问题、重听片段、项目和用户标签
- 重新转录、重新总结、重建索引和导出均不得覆盖已有个人笔记
- 人类索引新增知识与个人笔记入口，同时保持总结稿和转录稿为主要阅读入口

## v4.11.0 (2026-08-29)
- 每期新作业新增结构化 `knowledge.json`，保存主题、实体、标签、关键洞察和可核验证据
- 正式报告新增「关键洞察与证据」要求；引述须存在于转录稿并与 segments 时间范围相符
- 新作业只有在报告、结构化知识和证据核验全部通过后才会从 `awaiting_report` 进入 `completed`
- 旧作业缺少 `knowledge_path` 时继续按旧规则核验，避免破坏历史结果

## v4.10.1 (2026-08-28)
- 索引改为显示总结完成时间与转录完成时间，不再使用节目发布日期排序
- 已有总结按核验完成时间倒序排列；缺少核验记录时回退到总结稿修改时间
- 尚未总结的条目按转录完成时间倒序排列，并排在已有总结条目之后
